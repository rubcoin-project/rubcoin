 ...
 *
 *   // network
 *   CRubcoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.rub-coin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CRubcoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CRubcoinNetwork(UniValue objIn)
    {
...
// // can be under: RubcoinNetwork
// //   -- signature requirements : Key1(User)
// class CRubcoinNetworkVariable : public CGovernanceObject
// {
// private:
